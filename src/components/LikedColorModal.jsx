...
import { getDatabase, ref, push, set } from "firebase/database"; // Importaciones de Firebase
...
const LikedColorModal = ({ onSave, onClick, color }) => {
  ...
  const handleSubmit = (event) => {
    ...
    const newColorRef = push(favoriteColorsRef); // Genera un ID único para el color

    set(newColorRef, newColor)
      .then(() => {
        console.log("Color guardado en Firebase correctamente");
        setIsSubmitting(false);
        onSave(); // Llamar a la función onSave para notificar que el color se guardó
        onClick(); // Cerrar el modal
      })
      .catch((error) => {
...
//     setTimeout(() => {
//       const savedColors =
//         JSON.parse(localStorage.getItem("colorFormData")) || [];
//       savedColors.push(newColor);
//       localStorage.setItem("colorFormData", JSON.stringify(savedColors));
//       setIsSubmitting(false);
